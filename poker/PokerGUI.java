...
public class PokerGUI extends JPanel {
	...
	static final int TABLE_WIDTH = Math.max(2 * TABLE_PADDING + (PokerGame.NUM_PLAYERS / 2) * X_OFFSET - CARD_PADDING - PLAYER_PADDING,
			52 * DECK_CARD_OFFSET + CARD_WIDTH + TABLE_PADDING);
	static final int COMMUNITY_OFFSET = TABLE_WIDTH/2 - (CARD_WIDTH * 5) / 2 - 2 * CARD_PADDING;
...
//		add(dealButton);
		...
		JFrame frame = new JFrame("Wacky Poker Simulator");
